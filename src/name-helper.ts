...
export class NameHelper {
    ...
    static isValidUniqueName(name: string) {
        return typeof name === 'string' && name.trim().length > 1 && name.replace(/\s+/g, ' ') === name;
    }

    /**
     * Detects if a name is irregular or not.
     * Irregular names:
     * 1. iPhone
     * 2. CIA
     * @param name Name to test
     */
    static isIrregular(name: string): boolean {
        name = name.trim()
        ...
        for (let word of words) {
            ...
            const startWithLowerCaseName = word[0].toLowerCase() + word.substr(1);
            // Name->name === name
            if (startWithLowerCaseName !== word.toLowerCase()) {
                return true;
            }
        }
